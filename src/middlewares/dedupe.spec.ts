...
describe("graphql-requester/dedupe", () => {
  ...
  it("should make multiple requests if options passed", done => {
    ...
    const expectedData = (num: number) => `test${num}`;
    const response = (num: number) => ({ data: `test${num}` });
  ...
  it("should be able to handle new request after request is finished", done => {
    ...
    request(params[0], params[1]).then(({ data: data1 }) => {
      expect(data1).toEqual(expectedData(1));

      request(params[0], params[1]).then(({ data: data2 }) => {
        expect(data2).toEqual(expectedData(2));
        done();
      });
    });
